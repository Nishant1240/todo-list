...
$query = "SELECT `task_id`, `task_priority`, `task_name`, `task_due_date`, `task_progress` FROM `$table` WHERE `task_progress`= 'Completed' AND `user_id`='$user_id';";
...
if (mysqli_num_rows($results) > 0) {
    while ($row = mysqli_fetch_assoc($results)) {
        ...
        <td class='task__progress text-success'>{$row['task_progress']}</td>
        <td>
